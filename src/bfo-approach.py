...
for _ in range(S):
    ...
    fit = fitness(val, sal)
    bacteria.append({'team': team, 'fit': fit, 'val': val, 'sal': sal, 'health': 0})
...
for ell in range(N_ed): #elimination-dispersal loop
    for k in range(N_re): #reproduction loop
        for j in range(N_c): #chemotaxis loop
            ...
            for i in range(S):
                ...
                if new_fit > bacteria[i]['fit']:
                    ...
                    # swim and keep going in the same direction if it's improving
                    m = 0
                    while m < N_s:
                        m += 1
                ...
                if bacteria[i]['fit'] > best_fit_overall and bacteria[i]['sal'] <= totalBudget:
                    ...
                    best_team_overall = {k_pos: list(v_pos) for k_pos, v_pos in bacteria[i]['team'].items()}
        ...
        for i in range(half_S):
            ...
            bacteria[i]['health'] = 0 # reset health for next generation
    ...
    for i in range(S):
        if random.random() < P_ed:
            ...
            val, sal = evaluate_team(team)
            fit = fitness(val, sal)
            bacteria[i] = {'team': team, 'fit': fit, 'val': val, 'sal': sal, 'health': 0}
