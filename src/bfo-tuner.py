...
totalBudget = 50000
pos_pools = { 'C': idx_C, '1B': idx_1B, '2B': idx_2B, '3B': idx_3B, 'SS': idx_SS, 'OF': idx_OF, 'SP': idx_SP }
...
def get_random_team():
    return {
        'C': [random.choice(idx_C)], '1B': [random.choice(idx_1B)], '2B': [random.choice(idx_2B)],
        '3B': [random.choice(idx_3B)], 'SS': [random.choice(idx_SS)], 'OF': random.sample(idx_OF, 3),
...
def fitness(val, sal):
    return val if sal <= totalBudget else val - (sal - totalBudget) * 10 
...
def run_bfo(S, N_c, N_s, N_re, N_ed, P_ed):
    ...
    for _ in range(S):
        ...
        bacteria.append({'team': team, 'fit': fitness(val, sal), 'val': val, 'sal': sal, 'health': 0})
    ...
    for ell in range(N_ed):
        for k in range(N_re):
            for j in range(N_c):
                for i in range(S):
                    ...
                    if new_fit > bacteria[i]['fit']:
                        bacteria[i].update({'team': new_team, 'fit': new_fit, 'val': new_val, 'sal': new_sal})
                        m = 0
                        while m < N_s:
                            m += 1
                            ...
                            if swim_fit > bacteria[i]['fit']:
                                bacteria[i].update({'team': swim_team, 'fit': swim_fit, 'val': swim_val, 'sal': swim_sal})
                            else:
                                break
                    bacteria[i]['health'] += bacteria[i]['fit']
                    
                    if bacteria[i]['fit'] > best_fit_overall and bacteria[i]['sal'] <= totalBudget:
            ...
            for i in range(half_S):
                bacteria[i + half_S] = {
                    'team': {k_pos: list(v_pos) for k_pos, v_pos in bacteria[i]['team'].items()},
                    'fit': bacteria[i]['fit'], 'val': bacteria[i]['val'], 'sal': bacteria[i]['sal'], 'health': 0
                }
                bacteria[i]['health'] = 0 
        ...
        for i in range(S):
            if random.random() < P_ed:
                team = get_random_team()
                val, sal = evaluate_team(team)
                bacteria[i] = {'team': team, 'fit': fitness(val, sal), 'val': val, 'sal': sal, 'health': 0}
...
if __name__ == "__main__":
    ...
    print("OPTUNA TUNING IN PROGRESSS")
    print("...\n")
    ...
    print("\n" + "="*50)
    print(" OPTUNA TUNING FINISHED > CHOSEN PARAMETERS ")
    print("="*50)
    for key, value in study.best_params.items():
        print(f"{key} = {value}")
    print(f"\nMAX FOUND VALUE: {study.best_value:.2f}")
    print("="*50)