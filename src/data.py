...
# first we take the batters who appear at the plate regurarly
# and we shorten the data bc the orignal dataset has over 200 stats
...
df_pitchers = pd.DataFrame(
    {
        ...
        "IP": [random.randint(50, 220) for _ in range(90)],
        "ERA": [round(random.uniform(2.50, 5.50), 2) for _ in range(90)],
        "SO": [random.randint(50, 250) for _ in range(90)],
        "WAR": [round(random.uniform(0.5, 6.0), 1) for _ in range(90)],
    }
)
...
df_batters["Salary"] = [
    random.randint(4000, 10000) for _ in range(len(df_batters))
]
...
df_pitchers["ERA_norm"] = (df_pitchers["ERA"].max() - df_pitchers["ERA"]) / (
    df_pitchers["ERA"].max() - df_pitchers["ERA"].min()
)
...
df_pitchers["Salary"] = [
    random.randint(4000, 12000) for _ in range(len(df_pitchers))
]

# Random positions for batters
positions = ["C", "1B", "2B", "3B", "SS", "OF"]
df_batters["Pos"] = [random.choice(positions) for _ in range(250)]
df_pitchers["Pos"] = "SP"
...
idx_SP = df_pitchers.index.tolist()
